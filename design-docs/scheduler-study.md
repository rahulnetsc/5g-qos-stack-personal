...
| — | `apply_demand_cap` | `False` | decided | Whether Tier-1 constrains `rᵢ ≤ Dᵢ`. Off: the cap was redundant with Tier-2's windowed ceiling, which does the same job from *observed* arrivals where the cap needed a *predicted* demand — and a collapsing cap is what turns an estimation error into unrecoverable starvation. |
| — | `demand_estimator` | `oracle` | deployment | How Tier-1 would obtain `Dᵢ` if the cap were on. Inert while `apply_demand_cap=False`; `tracking` remains useful as a bounded demand figure for admission control. |
