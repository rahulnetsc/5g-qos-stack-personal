...
typedef struct UEsched_s {
  ...
  bool has_gbr;          // ADD
  int gbr_bytes_slot;
  bool has_srb;          // LCG0/SRB data -> top tier
  bool liveness;         // SR/inactivity keepalive -> sort boost only
  bool needs_service;    // has pending work (for follower reservation)
} UEsched_t;
...
static int comparator(const void *p, const void *q)
{
  ...
  /* T1: SRB (LCG0) — control plane first */
  if (pp->has_srb && !qq->has_srb) return -1;
  if (!pp->has_srb && qq->has_srb) return 1;

  /* T2: liveness (SR/inactivity keepalive) — sort boost; min_rb floor in alloc */
  if (pp->liveness && !qq->liveness) return -1;
  if (!pp->liveness && qq->liveness) return 1;

  /* T3: GBR with unfulfilled deficit before non-GBR */
  if (pp->has_gbr && !qq->has_gbr) return -1;
  if (!pp->has_gbr && qq->has_gbr) return 1;

  /* T4 (lowest): pure inactivity/SR no-data UEs LAST (was -1 = front, a bug) */
  if (pp->sched_inactive && !qq->sched_inactive) return 1;
  if (!pp->sched_inactive && qq->sched_inactive) return -1;

  /* within tier: tighter remaining PDB first */
  if (pp->pdb_ms < qq->pdb_ms) return -1;
  if (pp->pdb_ms > qq->pdb_ms) return 1;

  /* same PDB: higher PF coefficient first (starved UE wins over saturating UE) */
...
const bool do_sched = nr_UE_is_to_be_scheduled(&nrmac->frame_structure,
    ...
    /* SRB data maps to LCG0 (get_SRB_RLC_BearerConfig sets logicalChannelGroup=0).
     * LCG0 buffer with no DRB on LCG0 == control/RRC pending -> top tier. */
    bool ul_has_srb = false;
    if (sched_ctrl->estimated_ul_buffer_per_lcg[0] > 0) {
      bool lcg0_is_drb = false;
      for (int _i = 0; _i < seq_arr_size(&sched_ctrl->lc_config); ++_i) {
        const nr_lc_config_t *_c = seq_arr_at(&sched_ctrl->lc_config, _i);
        if (_c->lcid >= 3 && (_c->lcid - 3) == 0) { lcg0_is_drb = true; break; }
      }
      if (!lcg0_is_drb) ul_has_srb = true;
    }
    ...
    sched_ctrl->ul_guaranteed_bytes      = 0;
    sched_ctrl->ul_be_bytes              = 0;
    ...
    for (int _lcg = 0; _lcg < 8; _lcg++) {
      ...
      for (int _i = 0; _i < seq_arr_size(&sched_ctrl->lc_config); _i++) {
        ...
        if (_c->gbr_ul_guaranteed > 0) {
          ...
          /* GBR floor is guaranteed; buffer beyond floor is best-effort */
          sched_ctrl->ul_guaranteed_bytes += _target;
          int _ovf = sched_ctrl->estimated_ul_buffer_per_lcg[_lcg] - _target;
          if (_ovf > 0) sched_ctrl->ul_be_bytes += _ovf;
        } else {
          /* non-GBR: entire buffer is best-effort (served from leftover) */
          sched_ctrl->ul_be_bytes +=
    ...
    /* pure proportional-fair within a tier; priority handled by sort tiers.
     * thr floored so a starved UE's coef rises (and wins over a saturating UE)
     * without blowing up. */
    const float _thr_ul = UE->ul_thr_ue > 1.0f ? UE->ul_thr_ue : 1.0f;
    float coeff_ue = (float)tbs / _thr_ul;
...
if (sched_ctrl->has_pending_gbr) {
  for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); ++i) {
    ...
    bool sched_inactive = (B == 0) && do_sched && !sched_ctrl->ul_has_unfulfilled_gbr;
...
UE_sched[numUE].gbr_bytes_slot = gbr_bytes_slot;               // ADDF
    UE_sched[numUE].has_srb = ul_has_srb;
    UE_sched[numUE].liveness = sched_inactive && !ul_has_srb;
    UE_sched[numUE].needs_service =
        (B > 0) || ul_has_srb || sched_ctrl->ul_has_unfulfilled_gbr || do_sched;
    numUE++;
  }
 ...
 for (int _di = 0; _di < numUE && UE_sched[_di].UE; _di++) {
  ...
  while (iterator->UE != NULL) {
    ...
    uint16_t max_rbSize = (iterator->sched_inactive || iterator->has_srb) ? min_rb : bi.bwpSize;
    /* reserve min_rb for each follower still needing service so a saturating BE
     * UE cannot zero a starved UE behind it (the 3-UE saturation case). */
    int n_followers_need = 0;
    for (UEsched_t *fwd = iterator + 1; fwd->UE != NULL; fwd++)
      if (fwd->needs_service) n_followers_need++;
    {
      int budget = (int)bi.bwpSize - n_followers_need * min_rb;
      if (budget < min_rb) budget = min_rb;
      if ((int)max_rbSize > budget) max_rbSize = budget;
    }
...
int B_eff = B;
{
  ...
  /* guaranteed (GBR floor) always requested; BE rides leftover RBs bounded by
   * the follower reservation above */
  int ul_target = _sc->ul_guaranteed_bytes + _sc->ul_be_bytes;
  ...
  /* control-only cap: SRB-tier UE drains LCG0 (SRB), not a large DRB BSR */
  if (iterator->has_srb) {
    int srb_target = _sc->estimated_ul_buffer_per_lcg[0];
    if (srb_target < 1) srb_target = 1;
    if (srb_target < ul_target) ul_target = srb_target;
  }
  B_eff = ul_target;
  LOG_D(NR_MAC,
        "[UL-QoS] UE %04x B=%d guar=%d be=%d B_eff=%d gbr=%d srb=%d\n",
        iterator->UE->rnti, B, _sc->ul_guaranteed_bytes, _sc->ul_be_bytes,
        B_eff, iterator->has_gbr, iterator->has_srb);
}
    if (iterator->has_gbr) {
  ...
  T_INT(sched.nrOfLayers));
    } else if (iterator->sched_inactive || iterator->has_srb) {
      // keepalive/control-only: SR, inactivity, or SRB with no data path
      sched.rbSize = min_rb;
...
void post_process_ulsch(gNB_MAC_INST *nr_mac, post_process_pusch_t *pusch, NR_UE_info_t *UE, NR_sched_pusch_t *sched_pusch)
{
  ...
  if (cur_harq->round == 0) {
    ...
    for (int _lcg = 0; _lcg < 8; _lcg++) {
      ...
      if (sched_ctrl->ul_lcg_deficit_bytes[_lcg] > 0) {
        /* distribute tb_size drain proportionally across active LCGs */
        sched_ctrl->ul_lcg_deficit_bytes[_lcg] -= sched_pusch->tb_size;
        if (sched_ctrl->ul_lcg_deficit_bytes[_lcg] < 0)
          sched_ctrl->ul_lcg_deficit_bytes[_lcg] = 0;
      }
    }
