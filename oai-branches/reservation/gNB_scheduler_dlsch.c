...
static uint32_t update_dlsch_buffer(frame_t frame, slot_t slot, NR_UE_info_t *UE)
{
  ...
  /* reset per-UE DL QoS summary — recomputed below across all active LCIDs */
  sched_ctrl->dl_best_remaining_pdb_ms = 9999;
  sched_ctrl->dl_best_priority = 90;
  sched_ctrl->dl_has_unfulfilled_gbr = false;
  sched_ctrl->dl_total_target_bytes = 0;
  sched_ctrl->dl_guaranteed_bytes = 0;
  sched_ctrl->dl_be_bytes = 0;
  ...
  for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); ++i) {
    ...
    /* ---- DL per-flow QoS: remaining PDB + deficit + target for this LCID ---- */
    if (lcid >= 4) {
      const NR_QoS_config_t *qc = &c->qos_config[0];
      const int pdb  = qc->pdb_ms  > 0   ? qc->pdb_ms  : 300;
      const int prio = qc->priority > 0   ? qc->priority : 90;
      const int spf  = 20; /* 30kHz: 20 slots/frame — matches your rancfg */
      const float slot_ms = 0.5f;

      /* remaining PDB using last drain stamp */
      int remaining_pdb = pdb;
      if (sched_ctrl->dl_lcid_last_drain_slot[lcid] > 0) {
        uint32_t now_abs  = (uint32_t)frame * spf + slot;
        uint32_t last     = sched_ctrl->dl_lcid_last_drain_slot[lcid];
        float    age_ms   = (float)((now_abs - last + 1024 * spf)
                                    % (1024 * spf)) * slot_ms;
        remaining_pdb = pdb - (int)age_ms;
        if (remaining_pdb < 0) remaining_pdb = 0;
      }

      /* update per-UE summary with most urgent active LCID */
      if (sched_ctrl->rlc_status[lcid].bytes_in_buffer > 0
          && remaining_pdb < sched_ctrl->dl_best_remaining_pdb_ms) {
        sched_ctrl->dl_best_remaining_pdb_ms = remaining_pdb;
        sched_ctrl->dl_best_priority = prio;
      }

      /* GBR deficit accumulation and target computation */
      if (c->gbr_dl_guaranteed > 0) {
        /* obligation this slot in bytes */
        int obligation = (int)(c->gbr_dl_guaranteed / 8) / (spf * 100);
        if (obligation < 1) obligation = 1;
        sched_ctrl->dl_lcid_deficit_bytes[lcid] += obligation;
        /* cap deficit at one PDB window worth to prevent unbounded catch-up */
        int window_bytes = obligation * (int)(pdb / slot_ms);
        if (sched_ctrl->dl_lcid_deficit_bytes[lcid] > window_bytes)
          sched_ctrl->dl_lcid_deficit_bytes[lcid] = window_bytes;

        if (sched_ctrl->dl_lcid_deficit_bytes[lcid] > 0)
          sched_ctrl->dl_has_unfulfilled_gbr = true;

        /* per-LCID target: spread deficit recovery across remaining deadline */
        if (sched_ctrl->rlc_status[lcid].bytes_in_buffer > 0) {
          int remaining_slots = (int)(remaining_pdb / slot_ms);
          if (remaining_slots < 1) remaining_slots = 1;
          int deficit  = sched_ctrl->dl_lcid_deficit_bytes[lcid];
          int target   = (deficit + obligation) / remaining_slots;
          if (target < obligation) target = obligation;
          /* cap at 2x per-slot MBR to prevent cell monopolisation */
          int max_burst = (int)(c->gbr_dl_max / 8) / (spf * 100) * 2;
          if (max_burst < obligation * 2) max_burst = obligation * 2;
          if (target > max_burst) target = max_burst;
          /* GBR floor guaranteed; buffer beyond floor is best-effort */
          sched_ctrl->dl_guaranteed_bytes += target;
          int ovf = sched_ctrl->rlc_status[lcid].bytes_in_buffer - target;
          if (ovf > 0) sched_ctrl->dl_be_bytes += ovf;
        }
      } else {
        /* non-GBR: entire buffer is best-effort (served from leftover) */
        sched_ctrl->dl_be_bytes +=
...
typedef struct UEsched_s {
  ...
  NR_UE_info_t *UE;
  int selected_mcs;
  int pdb_ms;   /* remaining PDB of most urgent active DL DRB */
  bool has_gbr; /* true if any GBR LCID has unfulfilled deficit */
  bool has_srb;        /* SRB1/SRB2 data -> top tier */
  bool liveness;       /* TA pending -> sort boost only */
  bool needs_service;  /* pending work (for follower reservation) */
} UEsched_t;
...
static int comparator(const void *p, const void *q)
{
  ...
  /* T1: SRB1/SRB2 — control plane above all DRB QoS */
  if (pp->has_srb && !qq->has_srb) return -1;
  if (!pp->has_srb && qq->has_srb) return  1;

  /* T2: liveness (TA) — sort boost; min floor in alloc */
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (harq_pid >= 0) {
      ...
      uint32_t tbs = nr_compute_tbs(Qm,
                                    ...
                                    0 /* tb_scaling */,
                                    l) >> 3;
    /* pure PF within a tier; priority handled by sort tiers */
      const float _thr_dl = UE->dl_thr_ue > 1.0f ? UE->dl_thr_ue : 1.0f;
      float coeff_ue = (float)tbs / _thr_dl;
      LOG_D(NR_MAC,
            "[DL-QoS] UE %04x b %d thr %.1f tbs %d prio %d rem_pdb %dms gbr %d coef %.4f\n",
            UE->rnti, b, UE->dl_thr_ue, tbs,
            sched_ctrl->dl_best_priority, sched_ctrl->dl_best_remaining_pdb_ms,
            sched_ctrl->dl_has_unfulfilled_gbr, coeff_ue);
      bool dl_has_srb = (sched_ctrl->rlc_status[1].bytes_in_buffer > 0)
                     || (sched_ctrl->rlc_status[2].bytes_in_buffer > 0);
      ...
      UE_sched[numUE].has_srb  = dl_has_srb;
      UE_sched[numUE].liveness = sched_ctrl->ta_apply && !dl_has_srb;
      UE_sched[numUE].needs_service =
          (sched_ctrl->num_total_bytes > 0) || dl_has_srb || sched_ctrl->ta_apply;
      numUE++;
    }
  }
  ...
  while (iterator->UE != NULL) {
   ...
   /* reserve min_rbSize for each follower still needing service so a saturating
     * BE UE cannot zero a starved UE behind it (3-UE saturation case). */
    ...
    {
      int budget = (int)max_rbSize - n_followers_need * min_rbSize;
      if (budget < min_rbSize) budget = min_rbSize;
...
const int oh = 3 * 4 + (sched_ctrl->ta_apply ? 2 : 0);
    ...
    /* use QoS-aware target: sum of per-LCID targets from update_dlsch_buffer.
     * For GBR flows this encodes deficit+obligation spread over remaining PDB.
     * For non-GBR flows it equals bytes_in_buffer — same as before. */
    int dl_target = sched_ctrl->dl_guaranteed_bytes + sched_ctrl->dl_be_bytes + oh;
    /* never request less than what is actually buffered (covers unclassified LCs) */
    if (dl_target < sched_ctrl->num_total_bytes + oh)
      dl_target = sched_ctrl->num_total_bytes + oh;

    /* control-only cap: SRB-tier UE drains SRB1/SRB2 (+oh), not full DRB buffer */
    if (iterator->has_srb) {
      int srb_bytes = sched_ctrl->rlc_status[1].bytes_in_buffer
                    + sched_ctrl->rlc_status[2].bytes_in_buffer;
      int floor_target = srb_bytes + oh;
      if (floor_target < dl_target) dl_target = floor_target;
    }
...
void post_process_dlsch(gNB_MAC_INST *nr_mac, post_process_pdsch_t *pdsch, NR_UE_info_t *UE, NR_sched_pdsch_t *sched_pdsch)
{
  ...
  if (harq->round != 0) { /* retransmission */
    ...
    if (sched_ctrl->num_total_bytes > 0) {
      /* TS 38.321 5.4.3.1: SRBs highest priority. Pass 0 drains SRB1 then SRB2
       * first; pass 1 drains DRBs in existing lc_config order (unchanged). */
      for (int _pass = 0; _pass < 2; ++_pass) {
      for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); ++i) {
        const nr_lc_config_t *c = seq_arr_at(&sched_ctrl->lc_config, i);
        const int lcid = c->lcid;

        const bool _is_srb = (lcid == 1 || lcid == 2);
        if (_pass == 0 && !_is_srb) continue;
        if (_pass == 1 && _is_srb)  continue;
        ...
        if (lcid_bytes > 0) {
          /* stamp when this LCID was last drained for HOL age tracking */
          sched_ctrl->dl_lcid_last_drain_slot[lcid] =
              (uint32_t)frame * nr_mac->frame_structure.numb_slots_frame + slot;
          /* drain GBR deficit by bytes actually delivered */
          if (sched_ctrl->dl_lcid_deficit_bytes[lcid] > 0) {
            sched_ctrl->dl_lcid_deficit_bytes[lcid] -= lcid_bytes;
            if (sched_ctrl->dl_lcid_deficit_bytes[lcid] < 0)
              sched_ctrl->dl_lcid_deficit_bytes[lcid] = 0;
          }
        }
      } /* end per-LC loop */
      } /* end _pass loop (SRB pass, then DRB pass) */
