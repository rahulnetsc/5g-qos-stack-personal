...
bool nr_mac_add_lcid(NR_UE_sched_ctrl_t* sched_ctrl, const nr_lc_config_t *c)
{
  ...
  qsort(base, nmemb, size, cmp_lc_config);
    ...
    /* DEBUG: dump lc_config order after sort */
  LOG_I(NR_MAC, "[LC-SORT-DEBUG] lc_config after sort (%zu entries):\n", nmemb);
  for (size_t i = 0; i < nmemb; i++) {
    const nr_lc_config_t *entry = (const nr_lc_config_t *)seq_arr_at(&sched_ctrl->lc_config, i);
    LOG_I(NR_MAC, "[LC-SORT-DEBUG]   [%zu] LCID %d | priority %d | suspended %d\n",
          i, entry->lcid, entry->priority, entry->suspended);
  }
