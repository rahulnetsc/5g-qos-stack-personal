...
#include "ia_p5g_scheduler.h"          /* TwoTier QoS scheduler — Checkpoint 1 */
...
uint32_t update_dlsch_buffer(frame_t frame, slot_t slot, NR_UE_info_t *UE)
{
    ...
    if (lcid >= 4) {
      ...
      if (c->gbr_dl_guaranteed > 0) {
        ...
        if (sched_ctrl->rlc_status[lcid].bytes_in_buffer > 0) {
          ...
          sched_ctrl->dl_total_target_bytes += target;
        }
      } else {
        /* non-GBR: target is simply what is in buffer — no obligation */
        sched_ctrl->dl_total_target_bytes +=
...
bool allocate_dl_retransmission(gNB_MAC_INST *nr_mac,
...
void ack_reconfig(gNB_MAC_INST *mac, NR_UE_info_t *UE)
{
...
static int comparator(const void *p, const void *q)
{
  ...
  /* GBR with unfulfilled deficit always before non-GBR — mirrors UL */
  if (pp->has_gbr && !qq->has_gbr) return -1;
  if (!pp->has_gbr && qq->has_gbr)  return  1;

  /* within same tier: tighter remaining deadline first */
  if (pp->pdb_ms < qq->pdb_ms) return -1;
  if (pp->pdb_ms > qq->pdb_ms) return  1;

  /* same remaining PDB: higher PF coeff first */
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (harq_pid >= 0) {
      ...
      uint32_t tbs = nr_compute_tbs(Qm,
                                    ...
                                    0 /* tb_scaling */,
                                    l) >> 3;
    int _prio = sched_ctrl->dl_best_priority > 0 ? sched_ctrl->dl_best_priority : 90;
      float coeff_ue = ((float)tbs / UE->dl_thr_ue) * (100.0f / (float)_prio);
      LOG_D(NR_MAC,
            "[DL-QoS] UE %04x b %d thr %.1f tbs %d prio %d rem_pdb %dms gbr %d coef %.4f\n",
            UE->rnti, b, UE->dl_thr_ue, tbs,
            _prio, sched_ctrl->dl_best_remaining_pdb_ms,
...
const int oh = 3 * 4 + (sched_ctrl->ta_apply ? 2 : 0);
    ...
    /* never request less than what is actually in the buffer */
...
static void nr_dlsch_preprocessor(gNB_MAC_INST *mac, post_process_pdsch_t *pp_pdsch)
{
  ...
  /* proportional fair scheduling algorithm — TwoTier active when sched_stateful_data != NULL */
  if (mac->sched_stateful_data != NULL)
    ia_p5g_pf_dl(mac, pp_pdsch, UE_info->connected_ue_list,
                 max_sched_ues, num_beams, n_rb_sched);
  else
    pf_dl(mac, pp_pdsch, UE_info->connected_ue_list,
          max_sched_ues, num_beams, n_rb_sched);
}
    ...
    if (sched_ctrl->num_total_bytes > 0) {
      ...
      for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); ++i) {
        ...
        while (bufEnd-buf > sizeof(NR_MAC_SUBHEADER_LONG) + 1 ) {
          ...
          /* TwoTier DL LCP fill: cap pull to per-LCID budget if active */
          rlc_buffer_occupancy_t effective_ndata = ndata;
          if (nr_mac->sched_stateful_data != NULL) {
            int budget = ia_p5g_get_lcid_budget(
                (ia_p5g_state_t *)nr_mac->sched_stateful_data, rnti, lcid);
            if (budget >= 0 && (int)effective_ndata > budget)
              effective_ndata = (rlc_buffer_occupancy_t)budget;
          }
