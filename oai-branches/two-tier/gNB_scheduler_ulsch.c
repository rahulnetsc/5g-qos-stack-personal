...
{
  sched_ctrl->best_pending_5qi = -1;
  sched_ctrl->best_pending_priority = 999;
  ...
  for (int lcg = 0; lcg < 8; lcg++) {
    ...
    int target_lcid = lcg + 3;
    for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); ++i) {
      const nr_lc_config_t *c = seq_arr_at(&sched_ctrl->lc_config, i);
         if (c->lcid == target_lcid) {
        const NR_QoS_config_t *qc = &c->qos_config[0];


        if (qc->priority < sched_ctrl->best_pending_priority) {
          sched_ctrl->best_pending_priority = qc->priority;
          sched_ctrl->best_pending_5qi = qc->fiveQI;
          sched_ctrl->best_pending_pdb_ms = qc->pdb_ms;
        }
        if (c->gbr_ul_max > 0)
          sched_ctrl->has_pending_gbr = true;
        break;
      }
    }
  }
...
int compute_ph_factor(int mu, int tbs_bits, int rb, int n_layers, int n_symbols, int n_dmrs, long *deltaMCS, bool include_bw)
{
  // 38.213 7.1.1
  ...
  while (pdu_len > 0) {
    ...
    switch (lcid) {
      ...
      case UL_SCH_LCID_S_BSR:
        ...
        memset(sched_ctrl->estimated_ul_buffer_per_lcg, 0, sizeof(sched_ctrl->estimated_ul_buffer_per_lcg));
        sched_ctrl->estimated_ul_buffer_per_lcg[((NR_BSR_SHORT *)ce_ptr)->LcgID] = sched_ctrl->estimated_ul_buffer;
        update_ul_qos_priority(sched_ctrl);
        T(T_DASH_BSR,
  T_INT(UE->rnti),
  T_INT(sched_ctrl->estimated_ul_buffer),
  T_INT(sched_ctrl->best_pending_5qi),
  T_INT(sched_ctrl->best_pending_priority),
  T_INT(sched_ctrl->best_pending_pdb_ms),
  T_INT(sched_ctrl->has_pending_gbr ? 1 : 0),
  T_INT(sched_ctrl->estimated_ul_buffer_per_lcg[0]),
  T_INT(sched_ctrl->estimated_ul_buffer_per_lcg[1]),
  T_INT(sched_ctrl->estimated_ul_buffer_per_lcg[2]),
  T_INT(sched_ctrl->estimated_ul_buffer_per_lcg[3]));
        break;
      ...
      case UL_SCH_LCID_L_BSR:
        ...
        sched_ctrl->sched_ul_bytes = 0;
        {
          NR_BSR_LONG *lbsr = (NR_BSR_LONG *)ce_ptr;
          bool active[8] = {lbsr->LcgID0!=0, lbsr->LcgID1!=0, lbsr->LcgID2!=0, lbsr->LcgID3!=0,
                            lbsr->LcgID4!=0, lbsr->LcgID5!=0, lbsr->LcgID6!=0, lbsr->LcgID7!=0};
          memset(sched_ctrl->estimated_ul_buffer_per_lcg, 0, sizeof(sched_ctrl->estimated_ul_buffer_per_lcg));
          uint8_t *pay = ((uint8_t *)lbsr) + 1;
          int m = 0;
          for (int n = 0; n < 8; n++) {
            if (!active[n]) continue;
            int idx = min(overestim_bsr_index(pay[m]), NR_LONG_BSR_TABLE_SIZE - 1);
            sched_ctrl->estimated_ul_buffer_per_lcg[n] = get_long_bsr_value(idx);
            m++;
          }
        }
        update_ul_qos_priority(sched_ctrl);
                              ...
                              bool crc_status)
{
  ...
  if (!crc_status) {
    ...
    LOG_D(NR_MAC,
          ...
          harq_pid,
          rnti);
          T(T_DASH_HARQ, T_INT(rnti), T_INT(harq_pid), T_INT(0), T_INT(harq->round));
    ...
    LOG_D(NR_MAC,
          ...
          rnti,
          harq_pid);
           T(T_DASH_HARQ, T_INT(rnti), T_INT(harq_pid), T_INT(1), T_INT(harq->round));
  } else {
    harq->round++;
    LOG_D(NR_MAC,
          ...
          harq_pid,
          rnti);
          T(T_DASH_HARQ, T_INT(rnti), T_INT(harq_pid), T_INT(2), T_INT(harq->round));
    add_tail_nr_list(&sched_ctrl->retrans_ul_harq, harq_pid);
                       ...
                       const uint16_t rssi)
{
  ...
  if (UE) {
    ...
    if (sduP != NULL) {
      ...
      T(T_DASH_RXDATA, T_INT(UE->rnti), T_INT(sdu_lenP), T_INT(frameP), T_INT(slotP));
    } else {
...
bool nr_UE_is_to_be_scheduled(const frame_structure_t *fs,
                                     NR_UE_info_t *UE,
                                     frame_t frame,
                                     slot_t slot,
                                 ...
                                 uint8_t *mcs)
{
  ...
  if (ph_limit < tx_power) {
    ...
    T(T_DASH_PHLIMIT, T_INT(ph_limit), T_INT(tx_power), T_INT(*Rb), T_INT(*mcs), T_INT(tbs_bits));
  }
}
...
static int comparator(const void *p, const void *q)
{
  ...
  /* both UEs are !sched_inactive — sort by PDB first (lower = more urgent) */
  if (pp->pdb_ms < qq->pdb_ms)
    return -1;
  else if (pp->pdb_ms > qq->pdb_ms)
    return 1;
  /* same PDB: fall back to PF coefficient */
  if (pp->coef < qq->coef)
    return 1;
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  T_INT(UE->rnti),
  T_INT(1),
  T_INT(sched_ctrl->ul_harq_processes[ul_harq_pid].sched_pusch.rbSize),
  T_INT(n_rb_sched[beam.idx]),
  T_INT(sched_ctrl->ul_harq_processes[ul_harq_pid].sched_pusch.tb_size),
  T_INT(sched_ctrl->has_pending_gbr ? 1 : 0));
    ...
    for (int _lcg = 0; _lcg < 8; _lcg++) {
      ...
      for (int _i = 0; _i < seq_arr_size(&sched_ctrl->lc_config); _i++) {
        ...
        /* remaining PDB using last grant stamp */
        int _rem_pdb = _pdb;
        ...
        if (_c->gbr_ul_guaranteed > 0) {
          ...
          /* target: spread deficit over remaining deadline */
          ...
          /* non-GBR: target is buffer estimate */
          sched_ctrl->ul_total_target_bytes +=
              sched_ctrl->estimated_ul_buffer_per_lcg[_lcg];
        }
        /* update best priority for coeff formula */
        if (_ppri < prio) prio = _ppri;
        break;
      }
    }
    /* silence detection: reset deficit if app stopped sending */
    ...
    float coeff_ue = ((float)tbs / UE->ul_thr_ue) * (100.0f / (float)prio);
...
if (sched_ctrl->has_pending_gbr) {
  for (int i = 0; i < seq_arr_size(&sched_ctrl->lc_config); ++i) {
     ...
     * Mirrored verbatim from ia_p5g_scheduler.c so BOTH branches carry the
     * same starvation fix and the base-vs-twotier comparison stays fair.
     * A UE with pending UL bytes (B > 0) but nothing on data LCGs 1-7
     * (SRB-only backlog on LCG 0, or SR-derived estimate without BSR
     * detail) is invisible to the GBR/PDB classification above and would
     * otherwise compete in the lowest class — starving under saturation
     * and wedging SRB1 (RLC STATUS / RRC Complete undeliverable).
     * Schedule it like sched_inactive: first class, one min_rb grant.   */
    ...
    bool sched_inactive = ((B == 0 && do_sched) || cp_floor)
                          && !sched_ctrl->ul_has_unfulfilled_gbr;

     /* removed - absorbed into DASH_SORT */
...
UE_sched[numUE].gbr_bytes_slot = gbr_bytes_slot;               // ADDF
    numUE++;
  }

  qsort(UE_sched, numUE, sizeof(UEsched_t), comparator);
 // --- DASHBOARD: sorted order after pf_ul first pass ---
 for (int _di = 0; _di < numUE && UE_sched[_di].UE; _di++) {
   NR_UE_sched_ctrl_t *_sc = &UE_sched[_di].UE->UE_sched_ctrl;
   T(T_DASH_SORT,
  T_INT(_di),
  T_INT(UE_sched[_di].UE->rnti),
  T_INT(UE_sched[_di].has_gbr ? 1 : 0),
  T_INT(UE_sched[_di].pdb_ms),
  T_INT(UE_sched[_di].gbr_bytes_slot),
  T_INT(UE_sched[_di].sched_inactive ? 1 : 0),
  T_INT(UE_sched[_di].selected_mcs),
  T_INT(max(0, _sc->estimated_ul_buffer - _sc->sched_ul_bytes)),
  T_INT(_sc->sched_ul_bytes),
  T_FLOAT((float)UE_sched[_di].coef));
 }
  ...
  while (iterator->UE != NULL) {
    ...
    if (rbStart + min_rb > bi.bwpSize || available_rb < min_rb) {
      ...
      reset_beam_status(&nrmac->beam_info, sched_frame, sched_slot, iterator->UE->UE_beam_index, slots_per_frame, beam.new_beam);
       T(T_DASH_RBTRACK,
  T_INT(iterator->UE->rnti),
  T_INT(2),
  T_INT(0),
  T_INT(n_rb_sched[beam.idx]),
  T_INT(0),
  T_INT(iterator->has_gbr ? 1 : 0));
      iterator++;
      continue;
    }
else
...
T(T_DASH_RADIO,
  T_INT(iterator->UE->rnti),
  T_INT(sched_ctrl->pusch_snrx10),
  T_INT(sched_ctrl->ph),
  T_INT(sched_ctrl->pcmax),
  T_INT(iterator->selected_mcs),
  T_FLOAT((float)sched_ctrl->ul_bler_stats.bler),
  T_FLOAT((float)iterator->UE->ul_thr_ue));
    ...
    /* GBR floor — but cap at 2x floor to avoid over-granting
 * when BSR is stale. This prevents starvation of non-GBR
 * while still guaranteeing the rate. */
int B_eff = B;
{
  NR_UE_sched_ctrl_t *_sc = &iterator->UE->UE_sched_ctrl;
  int ul_target = _sc->ul_total_target_bytes;
  /* never request less than actual estimated buffer */
  if (ul_target < B) ul_target = B;
  /* legacy floor for GBR keepalive when BSR is stale */
  ...
  B_eff = ul_target;
  LOG_D(NR_MAC,
        "[UL-QoS] UE %04x B=%d ul_target=%d B_eff=%d gbr=%d\n",
        iterator->UE->rnti, B, ul_target, B_eff, iterator->has_gbr);
}
    if (iterator->has_gbr) {
  /* removed - absorbed into DASH_ALLOC */
}
...
void post_process_ulsch(gNB_MAC_INST *nr_mac, post_process_pusch_t *pusch, NR_UE_info_t *UE, NR_sched_pusch_t *sched_pusch)
{
  ...
  if (cur_harq->round == 0) {
    ...
    /* stamp grant time per LCG and drain GBR deficit.
     *
     * The gNB cannot observe the UE's actual per-LCID LCP fill (TS 38.321
     * §5.4.3.1 is UE-side), so we MODEL it: walk the active LCGs in strict
     * 3GPP-priority order (lowest priority number first) and consume the
     * granted tb_size against each LCG's reported buffer, exactly as the
     * UE's strict-priority LCP would. An LCG is credited only with what the
     * TB could actually have delivered to it after all higher-priority LCGs
     * took their share.
     *
     * TWO things key off the modelled per-LCG served bytes:
     *   (1) GBR deficit drain — reduce each LCG's deficit by what it plausibly
     *       received, NOT by the full tb_size (the old bug: every backlogged
     *       LCG was credited the entire TB, so a lower-priority GBR flow's
     *       deficit was wrongly cleared and it looked satisfied while starved).
     *   (2) grant-age stamp (Defect 2) — reset ul_lcg_last_grant_slot ONLY for
     *       LCGs that actually received bytes this TB. Stamping every active
     *       LCG (the old behaviour) falsely resets the deadline-urgency proxy
     *       on flows the UE left unserved, hiding their lateness from the
     *       Tier-2 metric exactly when it should escalate them.               */
    const uint32_t grant_slot_abs = (uint32_t)sched_pusch->frame
                                    * nr_mac->frame_structure.numb_slots_frame
                                    + sched_pusch->slot;

    /* Build the active-LCG set (buffer > 0), tagged with LCP priority. */
    typedef struct { int lcg; int priority; } lcg_prio_t;
    lcg_prio_t order[8];
    int n_active = 0;
    for (int _lcg = 0; _lcg < 8; _lcg++) {
      if (sched_ctrl->estimated_ul_buffer_per_lcg[_lcg] <= 0) continue;
      int target_lcid = _lcg + 3;
      int prio = 90; /* fallback if lookup fails */
      for (int _i = 0; _i < seq_arr_size(&sched_ctrl->lc_config); ++_i) {
        const nr_lc_config_t *c = seq_arr_at(&sched_ctrl->lc_config, _i);
        if (c->lcid == target_lcid) {
          prio = c->qos_config[0].priority;
          break;
        }
      }
      order[n_active].lcg = _lcg;
      order[n_active].priority = prio;
      n_active++;
    }

    /* Sort ascending priority number = served first (strict-priority LCP). */
    for (int i = 0; i < n_active; i++)
      for (int j = i + 1; j < n_active; j++)
        if (order[j].priority < order[i].priority) {
          lcg_prio_t tmp = order[i]; order[i] = order[j]; order[j] = tmp;
        }

    /* Greedy walk: consume tb_size against each LCG's buffer in priority
     * order. Credit deficit and stamp grant-age ONLY where served > 0. */
    int remaining_tb = sched_pusch->tb_size;
    for (int i = 0; i < n_active && remaining_tb > 0; i++) {
      int lcg = order[i].lcg;
      int available = sched_ctrl->estimated_ul_buffer_per_lcg[lcg];
      int served = min(remaining_tb, available);
      if (served <= 0) continue;

      /* (2) Defect 2: grant-age reset only for LCGs that got bytes. */
      sched_ctrl->ul_lcg_last_grant_slot[lcg] = grant_slot_abs;

      /* (1) drain GBR deficit by modelled served bytes, not full TB. */
      if (sched_ctrl->ul_lcg_deficit_bytes[lcg] > 0) {
        sched_ctrl->ul_lcg_deficit_bytes[lcg] -= served;
        if (sched_ctrl->ul_lcg_deficit_bytes[lcg] < 0)
          sched_ctrl->ul_lcg_deficit_bytes[lcg] = 0;
      }
      remaining_tb -= served;
    }
  ...
  T_INT(sched_ctrl->has_pending_gbr ? 1 : 0),
  T_INT(sched_ctrl->estimated_ul_buffer),
  T_INT(sched_ctrl->sched_ul_bytes));
...
static void nr_ulsch_preprocessor(gNB_MAC_INST *nr_mac, post_process_pusch_t *pp_pusch)
{
  ...
  while (max_dci > 0) {
    ...
    *next = fs_get_max(fs, *next, min_next);
    ...
    int sched;
    if (nr_mac->sched_stateful_data != NULL)
      sched = ia_p5g_pf_ul(nr_mac, pp_pusch, tda, tda_info,
                            nr_mac->UE_info.connected_ue_list,
                            max_dci, num_beams, len);
    else
      sched = pf_ul(nr_mac, pp_pusch, tda, tda_info,
                    nr_mac->UE_info.connected_ue_list,
                    max_dci, num_beams, len);
