...
| 4a | **New checklist item, added scoping commit 3's own end-of-commit honest-assessment review, not deferred to a bare README flag.** Wire `guaranteed_bytes + be_bytes` (commit 3's own output, currently computed but unconsumed) into grant *sizing* as the `nr_find_nb_rb`-equivalent target (`ul_target`/`dl_target`, `gNB_scheduler_ulsch.c:2496`/`_dlsch.c:1009`), replacing the current backlog-only sizing (`ceil(backlog*8/bits_per_rb)`, identical in mechanism to `sim/baselines/pf.py`'s own sizing). Sequenced alongside commit 4 because follower budget also needs a demand estimate and the two may share plumbing — order between 4 and 4a to be decided when reached, not implied by this row's position. Until this lands, reservation's grant *size* is behaviorally indistinguishable from PF's, even though its *ranking* (commit 2's real GBR/PDB tiers) already is not — see the honest-assessment note preserved in session history for the full reasoning. | Inert (not yet built). |
...
Reservation commits 1, 2, 3 and 3a landed; two-tier not started. Two user
decisions (D1, D2) obtained directly and recorded above before any code,
matching `docs/wp-join-plan.md`'s D0a/D0b precedent. Sequencing (D4):
reservation first, two-tier second.

Next: commit 4 (follower budget) and 4a (grant-sizing target). Their
order is deliberately left open in the checklist above — 4a first is the
stronger sequence, since the follower budget caps a demand estimate and,
until 4a lands, that estimate is still backlog-only, so commit 4's tests
could not distinguish a correct cap from a correct cap on the wrong
quantity. That is the same failure shape as commit 2's `pdb_ms` bug and
commit 3's float-vs-int one: a test that agrees with ground truth at the
sampled points and diverges everywhere else. Commit 4's own acceptance
criterion (`n_followers=0` → unconstrained budget) is a degeneracy
identity that holds under either sequence, so it loses nothing by
waiting.
...
implementation and the right one agree.
