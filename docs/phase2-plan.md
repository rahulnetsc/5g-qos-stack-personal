...
VQ, replacing the bootstrap PF coefficient in both directions), and 4
(the UL service-interval floor's state machine plus the new Tier 1.5
comparator slot it requires) have now landed — see this table's own
row 1/row 2/row 3/row 3a/row 4 entries above for what each did and
confirmed; commit 4a and commits 5-9 not started. Two user decisions
...
unconstructed scenario. `README.md` §7's own new entry states this
plainly and names what would close it (a scenario that drives
`estimated_ul_buffer_per_lcg` to zero while real backlog persists);
`README.md` §10 also flags the consequence for commit 10's own study —
the mechanism most specific to two-tier's own design, born from a
documented production incident, currently unexercised by any scenario
in the regime map. Full suite: 470 passed (31 new tests, one
pre-existing rank-key assertion updated for the 4-tuple key shape, the
`min_rb`-kwarg signature-drift guard updated per its own standing
instruction).
