...
|---|---|---|---|---|
| **R1** | **⚠ SUPERSEDED 2026-09-08 — the ordering is NOT SCOREABLE for a stronger reason, and `[2,4]` is WITHDRAWN.** The 5QI-2 camera offered 3.8788 Mbps against its own 4.0000 Mbps GFBR, so the class that appeared to break first was the only one that could not pass by construction. Fixed at source; with the shortfall removed **nothing breaches at all on ×0.5–×2.0 at the deployment's cap** (`docs/g12-slide-source.md` §4). The declaration-order finding below stands as a control result and is kept. **G12's ordering is a declaration-order artefact.** Permutations 101/102/103 give `[2,4]` on all 5 seeds; **104 gives `[4,2]` on all 5**. | Not a scheduler property. | **X — a registered control that fired.** The strongest negative result here. | **Does not transfer — and must not be quoted as a scheduler behaviour.** Also additively biased by provisioning: 4 of 5 camera flows are offered **below their own GFBR** (3.879 vs 4.000 Mbps). |
