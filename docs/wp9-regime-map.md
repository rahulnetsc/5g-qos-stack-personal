...
| **G9** | **unrun, buildable — and now with a MEASURED argument** | M18 and M19 have read `pending` on **every row of every stage** (1,770 + 7,560 + 1,440) because no WP9 scenario configures `UEConfig.join`. **Nothing but a join campaign can move them** (§21.2a) — measured, not assumed. |
