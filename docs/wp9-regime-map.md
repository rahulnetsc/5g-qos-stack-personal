...
|---|---|---|
| **G1** | **partially answered** | M01 p98 / M15 across the core plane; ordering only, the millisecond is not certifiable (`SIM→RF`). TwoTier breaches its own PDB bound on 6/40 base-cell seeds (§29) — a G1 finding, unexamined. |
...
| **G3** | **partially answered, conditional** | M03/M14 at `t_live_s` ∈ {1,2,4}, reported as a function of it since `T_live` is `[OPEN: HARDWARE]`. M03 now carries a **cadence caveat** where a flow's own period exceeds the bound (Step 4). |
...
| **G6** | **ANSWERED — passes** | Both clauses, protected fleet, every arm, n_seeds=40 paired. **But G6 as *written* is unscoreable** — §0.6.1–0.6.3. |
...
| **G9** | **run — 1 clause scored, 1 scored via a companion metric, 1 weak-form only, 1 not producible** | The four clauses have four different answers and the row above states them separately. §21.2a's argument is discharged: M18/M19 flip out of `pending` the moment a join event exists, and M18/M21 now carry real numbers. **Open, each needing its own commit:** TwoTier's self-selected event shortfall (§34.5), and the unexplained neighbour Δp98 (§34.2). |
| **G10** | **ANSWERED — the headline** | Admissible N bounded by **8** at load ≥ 1.0 and **16** below it, on this RAN at `min_rb=5`. §0.1 and §0.3 apply. |
| **G11** | **unrun, buildable** | The soak was budgeted and never implemented. No WP9 evidence. |
| **G12** | **unrun — requirement now SPECIFIC** | Not "was not analysed": **M13 cannot be computed on any workload this WP ran**, because every one has exactly **one** GBR 5QI class and `first_violation_order` orders classes against each other. Needs **a workload with ≥ 2 GBR classes** — scenario work, not analysis. Widening M13 to the Delay-class flows would be redefining a pre-registered metric. |
...
no aggressor — and G6, which passes on measurement while being
**unscoreable as specified**, a finding about the guarantee rather than
