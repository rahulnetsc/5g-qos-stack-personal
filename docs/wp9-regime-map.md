...
> binds before PRB and TwoTier meets **30/30** deadlines against PF's
> **2/30** via Configured Grants, worst p99 15.0 → 5.0 ms
> (`adoption-decision.md` §1; `simulator-design.md` §9 records that SPS
> showed no benefit at all until the CCE budget was modelled, and that the
> regime narrows as DCI aggregation level varies with SNR).
>
> **What is corrected is one specific claim: G10's admissible-fleet boundary
> at 8 robots is NOT explained by the PDCCH bound on the PARAMETRIC mix.**
> Measured there, CCE utilisation is **9.4 % / 7.6 % / 7.3 % at N=8 / 16 /
> 32** while UL PRB sits at **93 %** — that workload is PRB-bound. The
> agreement between `32 CCE ÷ AL 4 = 8` and the observed boundary was
> **coincidental**, and treating it as confirmation was wrong.
>
> **THE BOUNDARY'S CAUSE IS OPEN.** The cold-start lock-out is a contributing
> mechanism — it floors M08 whenever it fires, and under the attach path
> `n_never_granted` goes to 0 at every fleet size. But it is **not
> established as the cause**: under that same attach path M08's minimum is
> ~0.89 at N=8 and ~0.77 at N=16, both below the 0.95 criterion, so the
> admissibility boundary does not simply move when the starvation is removed.
> **Stated as open rather than re-attributed.**
...
**AND THE MECHANISM IS NOW MEASURED, WHICH SHARPENS WHAT THE SCORE MEANS.
The gate ranks by `|mean delta| / sd`, so it is maximised by an axis that
PINS ONE ARM AT A FLOOR — not by an axis that moves the boundary.** When an
arm saturates, `sd` collapses and the ratio explodes. Recomputed from the
committed `stage1_rows.csv` (`scripts/regen_gate_verdict.py`, 2026-09-04):
PF's worst-flow GFBR fraction sits at **0.9618 in every cell of every axis**,
while Reservation moves between 0.0000 and 0.6723 — so the score is
measuring Reservation's distance from a floor, and it is largest exactly
where that floor is *hit on every seed*. `min_rb=1` and `sr_period_slots=1`
score an **identical 152.579** because both drive Reservation to exactly
**0.0000 on all 10 seeds** (`sd = 0.006`); the four `inf` scores above them
are `sd` of exactly zero on `M07.met`.

**So "11 of 12 cleared the bar" measures SEPARATION STRENGTH, not
RELEVANCE**, and the two come apart precisely at saturation: an axis that
pushes one arm off a cliff outranks an axis with a larger but noisier effect
on the thing the sweep is about. That is why the threshold did not
discriminate — not because the bar was set too low, but because the
statistic it is a bar on does not order axes by importance. **Read the
promotion decision below with that in mind:** the cap chose among axes
ranked by a quantity that rewards a pinned arm, and `min_rb`'s 152.579 is a
statement that `min_rb=1` starves Reservation completely, not that `min_rb`
governs the regime boundary — which §0.3 records as untested.
...
| **G3** | **Sim-informative, conditional — and G12's campaign supplies a telemetry-liveness failure it should be read against** | M03 scored at `t_live_s` ∈ {1, 2, 4} — reported as a function of it, since `T_live` is `[OPEN: HARDWARE]` and unmeasured. **CORRECTED 2026-09-04: this row previously read "M03/M14". M14 does NOT read `t_live_s`** — its threshold is `pdb_ms + survival_time_ms`, which its own docstring describes as used *"instead of T_live-derived ones"* — so it is not a function of it and is not G3 evidence. M14 binds to G11 alone. **Arrived from G12's campaign** (`docs/wp9-plan.md` §36.2): on that ramp the telemetry flow stops completing messages entirely, so its liveness gap becomes **unmeasurable on 116 ramp points** — a flow with no completions has no gap between completions. **That is the M19-shaped blindness on M03's own currency**: the gap statistic goes silent exactly where the failure is total, and M02 is the instrument with range. Any G3 reading on a starved flow must be taken with its M02, never alone. See G1's row for the failure itself. |
...
| **G7** | **NOT ANSWERABLE IN SIM** | No MFBR enforcement exists anywhere in `sim/` (`sim/config_loader.py:16`). Containment is observable; **clipping is not**, and clipping is half of G7's pass criterion. GT-4.3 is the only test.  **MECHANISM ADDED 2026-09-06 — PF's containment is NOT a fairness property of its ranking: it grants ~5.8x LESS (2,048 vs 11,902 UL grants to the same UE), so its protected flow rides on 5.1x more of them (2.74 % vs 0.54 %). Same grant-density mechanism as the workload inversion, seen from the side where it helps; the effect is UE-side LCP, identical in all three arms, and no scheduler change reaches it. `docs/grant-density-mechanism-2026-09-06.md`.** |
...
| **G10** | **⚠ SUPERSEDED 2026-09-07 → PF 6 / Res 6 / TT 5 (see the note above this table).** **Sim-answerable — the headline, and it is PER ARM** | **Admissible fleet size: PF 8 robots, Reservation 4, TwoTier 4** — the pre-registered per-seed all-pass read, computed by `scripts/g10_admissible.py` (§8d D4-3a). Four robots run clean on every arm; at eight, PF passes 10/10 seeds while Reservation passes 3/10 and TwoTier 1/10. This is what simulation buys that the N=2 testbed cannot. **The 8/16 pair previously written here is D4-3's arm-SEPARATION boundary — a different quantity, and not per-arm.** §0.1 and §0.3 apply. |
...
|---|---|---|
| **G1** | **partially answered — plus a MEASURED telemetry PDB failure from G12's campaign** | M01 p98 / M15 across the core plane; ordering only, the millisecond is not certifiable (`SIM→RF`). TwoTier breaches its own PDB bound on 6/40 base-cell seeds (§29) — a G1 finding, unexamined. **And G12's ramp produced telemetry M02 = 1.000 with bg still flowing — TwoTier from nominal load (§36.2).** **RE-MEASURED 2026-09-04 on a second composition Phase 2 never reached: on `drone_heavy_n8` TwoTier reaches M02 = 1.000 at ×1.0 — nominal load, no overload applied — with 5QI 9 still carrying 17.4 Mbps, on 10/10 seeds (`docs/verification-2026-09-04.md`). The gap statistic is blind on 240 ramp points, so read with M02 only.** |
...
| **G3** | **partially answered, conditional** | M03 at `t_live_s` ∈ {1,2,4}, reported as a function of it since `T_live` is `[OPEN: HARDWARE]` (**M14 removed 2026-09-04: it does not read `t_live_s` and binds to G11, not G3**). M03 now carries a **cadence caveat** where a flow's own period exceeds the bound (Step 4). **From G12's campaign: the liveness gap is UNMEASURABLE on a flow that has stopped completing** — 116 ramp points — so it goes blind precisely where the failure is total (§36.2). Read with M02. **AND THE TwoTier VERDICT IS UNRESOLVABLE AT THIS PRECISION BY MORE SEEDS — do not read INCONCLUSIVE as "needs a larger n".** Tier-1's SCA loop reaches no fixed point on 41 of 50 solves, so the iteration cap picks a point on a limit cycle; moving that cap by ONE iteration moves M03 by **87 % of the entire TwoTier-vs-PF difference** (`sweeps/phase2/sca-convergence-2026-09-04/`). That is a **within-seed** source and every interval here is a bootstrap **over seeds**, so it is absent from the interval entirely and adding seeds does not average it away. **What WOULD resolve it: a fixed vertex-selection rule for the degenerate LP** — a lexicographic tie-break or a small secondary objective — which is a **behaviour change to the scheduler**, would move the regression corpus deliberately, and needs its own plan and its own check against ground truth. Until then G3's TwoTier reading is bounded by the solver, not by the sample. |
...
| **G6** | **NOT PUBLISHABLE — partially answered at best; the guarantee names no estimator** | **SUPERSEDED 2026-09-04.** Its verdict depends on a choice the clause declines to make: on one cell the median reads PASS, the mean INCONCLUSIVE and a per-run conjunction FAIL, and the per-run reading also moves with the (unconfirmed) run count. **No G6 verdict is published** (`docs/verification-2026-09-04.md`, specification finding 4). The text below is the pre-correction reading and is retained for the record only. | Clause 1 ("stays within its bound") passes on every arm. Clause 2 passes on **M02** on every arm; on **M20** it passes on PF and Reservation and is **INCONCLUSIVE on TwoTier** (+29.35 % [+4.81, +56.18] — interval excludes zero, straddles the +20 % bar). Protected fleet, n_seeds=40 paired, one cell. **And G6 as *written* is unscoreable** — §0.6.1–0.6.3. |
...
| **G8** | **partially answered — and the published PASS is KNOWN-WRONG** | **⛔ 2026-09-04: at n=10 BOTH conjuncts FAIL on both QoS-aware arms** — M09 below 0.90 on 1/10 Reservation and 3/10 TwoTier seeds, M22 starvation epochs on 7/10 and 3/10, longest 10.0 s against a 1 s bar (`docs/verification-2026-09-04.md` §1). The prior pass came from an n=3 artefact whose own file already recorded a 10-second starvation the row denied. Not a code change — isolated at the published configuration. Original text: M09 per-second Jain across the sweeps; **PF arm contaminated** by `pf.py`'s declaration-order tie-break — Reservation-vs-TwoTier is the trustworthy pair. |
...
**Read as a whole: 2 answered (G4, G10), 1 measured failure (G5),
5 partially answered (G1, G3, G6, G8, G11), 2 run with clause-level
answers (G9, G12), 1 blocked on a named mechanism (G2), 1 structurally
out (G7).**
...
| H | Verdict | Basis |
|---|---|---|
| **H1** (reservation collapses above a UE count) | **Confirmed; the BOUND'S CAUSE IS NOW OPEN, corrected 2026-09-05** | Boundary at N=8 / N=16 — **previously attributed to the PDCCH bound, which does not bind ON THIS WORKLOAD** (CCE utilisation 7.3–9.4 % against UL PRB 93 %), so the agreement with `32/4 = 8` was coincidental. **PDCCH does bind elsewhere** — `sensor_dense`, 30 periodic UL sensors, TwoTier 30/30 vs PF 2/30 via Configured Grants (`adoption-decision.md` §1) — so the bound is real and the error was applying it here. **The cause of THIS boundary is open**: the cold-start lock-out contributes (it floors M08 when it fires) but does not explain it, since under the attach path M08's minimum is still ~0.89 at N=8 and ~0.77 at N=16, below the 0.95 criterion. §0.3 limits it to `min_rb=5`. |
