...
| **3** | *(registered BEFORE running — commit 2 showed the blind/binds call is worth predicting, not assuming: it registered "blind", the commit reached `RunRecord`, and `--check` failed on first run.)* **PREDICTED for commit 3: `--check` BINDS AND MOVES.** The corpus stores `RunRecord.timeseries_time_s` / `timeseries_slot_index` and every per-flow `ts_*` array; a per-second fold rewrites all of them. **Expected movement: those arrays shorten from `horizon_slots` to `ceil(horizon_s)` entries on every one of the 20 records** — a shape change, not a numeric drift. **M09 and M08w must be bit-identical; M04/M19/M21 change and are re-baselined deliberately.** If `--check` comes back CLEAN, that is the finding — it would mean the fold is not reaching the serialised record. **Per-second timeseries fold** (§4.2) — removes the other half. M09 and M08w preserved *exactly*; M04/M19/M21 lose per-slot resolution. | **`--check` BINDS PARTIALLY.** The fold changes `RunRecord.timeseries_*`, which the corpus *does* store — so `--check` **will** move, and that is expected, not a regression. Binding guards: M09/M08w bit-identical (they consume per-second buckets), M04/M19/M21 move and are re-baselined deliberately |
