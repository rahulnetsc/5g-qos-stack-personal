...
**Status: complete — all ten guarantees, five arms, plain / `+CG` /
`+CGt`, 43 500 runs (2026-09-16 04:08).** Every number comes from an
artefact under `sweeps/cell-2026-09-15/` (2026-09-15, Windows, four arms)
or `sweeps/cell-2026-09-16-linux/` (2026-09-16, Linux, five arms and the
CG set) on the code in §0. This document supersedes `docs/results-aligned-2026-09-14.md`
...
| `cg/g12_cg.json` | 400 sweeps (4 400 runs) | 531 s | done |
...
### 10.5 With configured grants (`cg/g12_cg.json`, 400 sweeps)

Clause 4 stays 10/10 on every CG arm, and the class orders are the plain
ones (PF `[4]`, `[4, 2]`; Reservation `[]`; ConfigSched `[]`, `[2]`) —
the CG carries no GBR class's bytes, so nothing about which class breaks
first can move. **What moves is the leading indicator, on exactly the two
arms that had one:** TwoTier's telemetry PDB-violation fraction at
×1.0 / ×1.4 / ×1.8 / ×2.0 goes 0.763 / 0.949 / 0.923 / 0.857 → **0.010 /
0.020 / 0.020 / 0.000** under `+CG`, and ConfigSched's 0.000 / 0.105 /
0.694 / 0.822 → **0.000 / 0.000 / 0.000 / 0.000**; `+CGt` reads 0.04–0.10
on TwoTier and ≤ 0.04 on ConfigSched. The heartbeat's lateness was the
dynamic path's, on both arms, and the ramp's growing video and camera
demand could crowd it only while it was there.

## 11. Product + configured grants on the deployed cell

Every guarantee was measured plain, `+CG` (restricted Type 2 on the
heartbeat, period from the PDB rule) and `+CGt` (period and phase from
the declared traffic) on all five arms — 30 950 runs in the CG set
against 12 550 plain. What the CG changes, per guarantee:

| G | what the CG does | what it does not touch |
|---|---|---|
| G1 | nothing (0 breaches on all ten CG arms; latencies within a slot) | a downlink instrument |
| G2 | nothing beyond seed noise | a downlink instrument; the BLER² floor is retry timing |
| G3 | **every arm 10/10 on all four parts to N = 24**; `+CGt` p98 4.5–12 ms | the camera's protected uplink (TwoTier 8.5 Mbps at N = 24 under both) |
| G4 | telemetry-after-silence 24–34 ms (`+CG`), **11–17 ms (`+CGt`)** on every arm | — |
| G5 | Reservation fleet 7 → 8 and GT-3.3 fail → pass; Proto 8 → 10; edge-robot telemetry 1/10 → 10/10 at −6/−3 dB | the video itself (±3 ms, ±0.02 on every arm) |
| G6 | the telemetry half on every arm: worst flood silence 6.5–9.6 s → ~200 ms | the camera half; G6 still fails everywhere |
| G7 | TwoTier's clause 1 (A's heartbeat 960 → 24 000 bps); Reservation's clause 2 1.87× → 1.18× | video containment as such; Reservation's A-camera p98 35 → 84 ms is the cost |
| G9 | TwoTier 4 → 11 of 12 informative cells; ConfigSched 9 → 12 | cold-attach first service +100–200 ms on every arm; the RLF caveat |
| G10 | **Reservation's cold-start lock-out removed** (never-granted 94 → 0, fleet 4 → 8); TwoTier 5 → 7 | PF, Proto, ConfigSched (8, 7, 10) |
| G12 | the telemetry indicator: TwoTier 0.76 → 0.01, ConfigSched 0.82 → 0.00 | the class orders |

**Two statements cover the table.** Every failure the faithful arms have
in the *heartbeat* class — G3's cadence, G6's flood silences, G7's
starved victim, G9's slow joiner, G10's lock-out, G12's late telemetry —
is removed on every arm by a configured grant that carries only that
bearer, with a cost of 100–200 ms on a cold attach and a reserved
occasion the camera cannot use. Nothing in the *video* class (G5's
fleet, G6's camera half, G7's containment) or the *downlink* class (G1,
G2) moves, because the CG never carries those bytes. The product's
uplink heartbeat problem is a bearer-configuration problem; its video
and downlink problems are the scheduler's.

## 12. Overall conclusion

**Per arm, plain, on the deployed cell** (the verdict, and the number
that decides it)

| G | PF | Reservation | TwoTier | ProtoRRageD2 | ConfigSched |
|---|---|---|---|---|---|
| G1 teleop | pass, 0/1 280 | pass | pass | pass | pass; slowest at cap 2 (10.5–20.5 ms) |
| G2 STOP | fail, 748 at cap 2 (demotion) | fail, 251 | fail, 247 | fail, 263 | fail, **1 390** at cap 2 |
| G3 liveness | pass to 24; all parts 10 | fail, 4 | fail, 6 | pass; 10 | pass; 10 |
| G4 post-silence | pass | pass | pass | pass | pass |
| G5 video | fleet 8, knee ×1.4 | 7, ×1.2, GT-3.3 fail | **none** | 8, no knee | 7, knee ×1.1 |
| G6 isolation | fail | fail | fail | fail | fail |
| G7 containment | pass, 0.67× | fail c2, 1.87× | fail c1, 4 % | pass, 0.82× | pass, 0.92×; best on all clauses |
| G9 join | 11/12 | 10/12 | **4/12** | **12/12** | 9/12 (cell breaks at ×1.25) |
| G10 fleet | 8 | 4 | 5 | 7 | **10** |
| G12 degradation | c4 pass; video first | c4 pass; nothing breaks | c4 pass; telemetry 76 % late at ×1.0 | c4 pass | c4 pass; `[]`/`[2]` |

1. **The cell moves every uplink boundary down and the STOP floor up,
   before any scheduler does anything.** With 64 % of the previous
   uplink: G3's part-3 boundary 24 → 10 on PF, G5's admissible fleet
   14 → 8, G10's 12 → 8; and the 5 ms STOP holds one HARQ retry instead
   of two, so every arm misses ten times as many. The four faithful-arm
   columns of the Windows run are byte-identical here on G1, G3 and G5,
   so none of this is platform.
2. **The faithful arms' verdicts are the previous cell's.** TwoTier
   fails G3, G5, G7 and G9 on service cadence (worst silences 8–10 s,
   the victim's heartbeat at 4 %, cold attaches of 1–4 s), and its G12
   telemetry is late on 76 % of bytes at nominal load. Reservation fails
   G3, G5's edge axis, G7's containment (1.87× MFBR) and G10 on the
   cold-start lock-out and the absence of any MFBR enforcement — product
   findings, not port defects. PF passes what a contract-blind scheduler
   can and fails where its single-term key demotes a robot (G2 at cap 2,
   G7 by luck of fairness).
3. **`ProtoRRageD2` is the best arm overall**, as before: G3 and G4
   with PF, G5 without a load knee to ×1.5, G9 12/12, G7 contained — with
   the known gap on G10 (7 against PF's 8) and no downlink change.
4. **The ConfigSched prototype is the best arm on G7 and G10 and
   PF-grade on G3, G4, G6 and G9 within capacity, and it fails where
   its five mechanisms say it will.** Its wins are structural — the
   GFBR floor *is* G10's statistic, the MFBR demand cap *is* G7's
   containment. Its losses are traced, each to a line: one visit per
   PDB for a source whose period equals its PDB (G1 cap 2, G12's
   indicator, G9's broken incumbent), an "early" order by next-due that
   a long-interval flow always loses (G1), no plan between re-solves
   for a 5 ms bearer (G2 cap 2: 1 390 misses), equal PRB shares with
   the best-effort filler above the floor (G5's ×1.1 knee, G6's windowed
   floor), and floors dropped by `ue_id` under overload (G10 past its
   boundary). None of these is a tuning; all five are what the v2
   formulation (`docs/config-scheduler-handoff.md` §8c) replaces.
5. **Product + CG splits the guarantee table in two** (§11): the
   heartbeat class is closed on every arm by the bearer configuration,
   and what remains — G2, G5, G6's camera half, G7's video containment —
   is where the schedulers still differ and where the next work is.

## 13. What this report does not cover

- **G8** (equal entitlement) and **G11** (the shift-long soak) — not run
  in this campaign.
- **GT-2.3's own buckets** (1 / 5 / 60 s pauses, p99 ≤ 300 ms): §4
  scores every message by its preceding gap on a 5 s run, not that
  clause.
- **G9's neighbours clause** (ε unspecified) and the top of its axis
  (derived from the previous cell's G10 boundary; (7, ×1.5) and
  (8, ×2.0) are past this cell's capacity on every arm).
- **G12 above ×2.0** and the specified order `[9, 4, 2]` — the
  background never falls below τ, so 5QI 9 never "breaks" and no
  order can match.
- **G2's miss floor attribution** is derived from the retry constants
  and the TDD pattern, not from per-TB HARQ counters.
- **The CG through RLF**: the model's CG persists and resumes on its old
  phase where a real gNB re-activates; G9's `+CG` rows are an upper
  bound.
- **Two MIMO layers**, **SR periodicity and k1/k2 from the deployed OAI
  source**, **held-out seeds**, and the CG's PRB-reservation cost as a
  separate line — all still open, as in the runbook.
- **ConfigSched is the prototype as committed plus two bug fixes**; the
  v2 formulation is designed, not built, and nothing about it is
  measured here.
