...
shows instead is the same fact as G5's load knee (×1.1) and G6's windowed
floor: **ConfigSched's cell breaks at ×1.25 of committed load where PF's
holds to ×1.5**, and G9 sees it from the incumbents' side. **The
neighbours clause remains untestable as written** (ε unspecified); at
ε = 1 ms no arm passes it above (4, ×0.75).
