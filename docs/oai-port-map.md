...
| 21 | UL GBR/BE byte split: obligation floored at 1, deficit accumulate/cap at one PDB-window, target spread capped at 2×max-burst, overflow beyond target to BE. Gated per-LCG on `estimated_ul_buffer_per_lcg > 0` for the *whole* block, including accumulation. | `gNB_scheduler_ulsch.c:2251-2278` (formulas), `:2230` (per-LCG gate) | `reservation.py::_ul_gbr_and_pdb` | `sim/tests/test_reservation.py::test_ul_obligation_floors_at_one_byte`, `::test_ul_deficit_caps_at_one_pdb_window`, `::test_ul_target_capped_at_2x_obligation_floor_without_mfbr`, `::test_ul_target_can_exceed_the_floor_when_mfbr_is_configured`, `::test_ul_overflow_beyond_target_credited_to_be`, `::test_ul_has_gbr_requires_a_real_gfbr_not_just_the_flow_class_label`, `::test_ul_deficit_freezes_when_the_per_lcg_estimate_is_zero` | Per-LCG, first-flow-found wins a shared LCG (matching the C's own `lc_config` linear-scan-then-`break`, `:2232-2234,2282`) — dormant, unexercised by any current scenario, the same H5 gap `README.md` §8 already names for BSR aliasing, now confirmed to affect deficit tracking too, not only short-BSR aliasing. `slots_per_sec` is derived from `self.slot_duration_s` rather than reading a hardcoded `spf` — see row 22's DL contrast. `guaranteed_bytes`/`be_bytes` are real but not yet consumed by grant sizing (`README.md` §8, `docs/phase2-plan.md` checklist item 4a). |
