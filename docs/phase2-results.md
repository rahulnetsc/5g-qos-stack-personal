...
| **G5** | **measured — and its VICTIM is index-determined; see `docs/g5-lever-2026-09-05.md`** | M05 completeness: PF FAIL 3/40, Reservation FAIL 30/40, TwoTier FAIL 34/40 under 0.99. M06 frame age: PF and Reservation PASS 0/40, **TwoTier FAIL 14/40** over 67 ms. **Scope: UL only on this workload** — M05/M06 read `frame_completions`, populated only for flows with a `frame_id`, which `sim/traffic.py` assigns only in `_gen_xr_video`; the parametric mix's one DL flow (qfi 82) has no PDU-set structure, so neither arm's DL ranking key can decide anything here. A reader will assume a completeness guarantee covers both directions; it does not. **And which UE fails is set by its POSITION in the flow declaration order, not by the UE**: all 8 UEs are configuration-identical, yet Reservation fails ue8 on 7/7 failing seeds and TwoTier fails {6,7,8} on 4/4, while PF (0/10) rotates freely. Reversing the list moves the failure to the mirrored positions; rotating by three moves it to ue7. The starvation is real (0 of 299 frames complete); its victim is arbitrary. |
