...
| **G9** | **STILL NOT SCOREABLE — but the cause is now identified; see `docs/g9-result-2026-09-05.md`** | Re-run 2026-09-05 on post-A1 scenarios. The count guard **still refuses**, now at *"4 'warm' events but the scenario schedules 10"* (was 2 of 10). **A1's horizon clip is inert here and that was derived, not assumed** — gt61's last event is slot 16,400 of 20,000, so every event was always reachable. **The change is attributable to the Tier-1 scaling, measured directly**: same scenario and seed, only `_OBJ_SCALE` differing, gives 2 warm events and 135 joiner UL grants at 1.0 against **8 events and 458 grants at 1e4**. **AND THE MECHANISM IS THE COLD-START LOCK-OUT** — the joiner sits with positive `bytes_reported` and an all-zero per-LCG array on **51.3 %** of its slots pre-scaling (17.2 % after), carrying `has_gbr=False`/`pdb_ms=9999` into the sort. **So G9's abort is a FOURTH observation of the mechanism behind G5, G10's boundary and the UL blackout: a re-attaching UE is a cold-starting UE.** Clauses 1–3 (M19/M18/M21 p95) remain **not scoreable** — a degenerate arm's surviving events are self-selected — and clause 4 was not reached. **Candidate fix identified, not started:** seed the BSR at `sim/join.py`'s `radio_connected_this_slot` edge, as Model C does at attach. |
