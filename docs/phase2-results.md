...
| **G5** | **RE-SCORED 2026-09-05 — its own registered falsifier fired; see `docs/attach-path-result-2026-09-05.md`** | **The published rate is an ARTEFACT of this simulator's missing attach procedure and must not be quoted as a property of either scheduler.** Published: PF 3/40, Reservation 30/40, TwoTier 34/40 failing M05 under 0.99. **Under an attach path (Model C: staggered arrival + the one BSR a UE sends during RRC setup), at G5's own configuration, 10 paired seeds: PF 0/10, Reservation 1/10 (marginal, M05 0.989967), TwoTier 0/10.** The catastrophic 0-of-299-frames failures are gone and the victim is no longer the last position. Starvation cleared at every fleet size, arm and seed (108-run experiment; the `stagger_only` control shows staggering ALONE makes it strictly WORSE, so the seed is the lever). **WHAT SURVIVES.** (a) The mechanism is unchanged and is the PRODUCT's: `ia_p5g_scheduler.c`'s Tier-1.5 floor is gated on `has_pending_gbr`, set only inside the loop that skips every zero per-LCG entry — the exact condition its own comment names as defining the fault it rescues — and Reservation has no floor at all. Read from the C's source, so no result here touches it. (b) The fault is reached MORE readily under realistic arrival than under cold start. (c) Model C models a *successful attach* and does not model a Short/Truncated-BSR desync emptying a served UE's array, which is the route hardware would actually take — **untested**. **Scope: UL only on this workload** (no DL flow on the parametric mix carries PDU-set structure). **Consequence: every blackout rate and admissible-fleet figure measured without an attach path is an UPPER BOUND**, G10's PF 8 / Reservation 4 / TwoTier 4 included. |
| **G6** | **NOT PUBLISHABLE — and DELIBERATELY NOT RE-RUN post-scaling** | Clause 1 within-bound and clause 2 shift ≤ +20 %, n=40 paired, protected fleet. **PF and Reservation byte-identical to the published values** — the control holds, so TwoTier's movement is attributable. TwoTier **worse**: M03 2→4/40, M06 12→14/40, M02 clause 2 PASS→**FAIL**. **G6 remains unscoreable as written** — the clause names no estimator over runs, and the three defensible readings give PASS, INCONCLUSIVE and FAIL on the same cell. **NOT overlooked in the 2026-09-05 post-scaling re-run: deliberately excluded.** No verdict is publishable regardless of scaling, so re-running it would produce three contradictory readings on new numbers instead of three on old ones — the same non-result at the cost of an n=40 campaign. Its numbers are PRE-scaling and are marked so in the provenance table. It becomes re-runnable the moment the clause specifies an estimator, and not before. |
| **G7** | **RECLASSIFIED 2026-09-05 — NOT structurally out; a SCENARIO gap. See `docs/g7-clipping-question-2026-09-05.md`** | **The previously recorded blocker was wrong on both halves.** The C **does** clip — three sites, both arms, both directions (`ia_p5g_scheduler.c:2663-2665`, `gNB_scheduler_ulsch.c:2248-2250`, `gNB_scheduler_dlsch.c:397-399`, the last carrying the comment *"cap at 2x per-slot MBR to prevent cell monopolisation"*) — and **the port implements all three faithfully** (`two_tier.py:1581-1585` and `1748-1752`, `reservation.py:1109-1113`), so there is **no divergence**. **But what is clipped is the GBR TARGET, not delivered bytes**: immediately after the clamp, `overflow = lcg_estimate - target; be_bytes += overflow` — excess is re-classified as best-effort and stays deliverable. Entitlement is a ceiling on the guarantee, not on throughput. GT-4.3's criterion is about **delivered** bytes, which are measurable today, so **G7 is scoreable once an aggressor scenario exists** (Asset B at 2× MFBR; confirmed absent, and `aggressor_multiplier` is never set outside tests). **Structural expectation, registered not scored:** nothing in either the C or the port bounds delivered rate at MFBR when spare capacity exists, so a FAIL on the middle clause is what the code points at — which would be a **product** finding, like the Tier-1.5 dead gate. |
...
| **G10** | **measured — UPPER BOUND, NOT A CAPACITY RESULT** | **⚠ PF 8 / Reservation 4 / TwoTier 4 IS AN UPPER BOUND on admissible fleet, not the number of robots a cell can host.** It was measured **without an attach path**, and the cold-start lock-out that sets the boundary (`docs/g5-mechanism-2026-09-05.md`) largely disappears once one exists — under Model C, starvation clears at every fleet size on every arm and seed (`docs/attach-path-result-2026-09-05.md`). **Do not present these as capacity figures**; a reader would size a deployment from them. The seed counts are the evidence and are also pre-attach: at N=8, PF **10/10**, TwoTier **6/10** (was 1/10), Reservation **3/10**. TwoTier's 6× improvement changes no verdict — see the criterion finding. Re-run post-scaling 2026-09-05: **verdict unchanged**. |
| **G11** | **ONE CLAUSE OF FIVE — C1 re-confirmed POST-SCALING** | C1 re-run 2026-09-05 on post-scaling code at GT-7.1's specified horizon: **30/30 runs at 7,200,000 slots, 1.000 on all three arms, 900 windows, 0 failing** — identical to the pre-scaling soak; PF and Reservation window counts bit-identical. **C2–C5 remain unscored**, so "G11 passes" would be quoting one clause of five. See below. |
