...
| **G5** | **measured — FAILURE STANDS; cause isolated to the deployed C's own ranking, see `docs/g5-mechanism-2026-09-05.md`** | M05 completeness: PF FAIL 3/40, Reservation FAIL 30/40, TwoTier FAIL 34/40 under 0.99. M06 frame age: PF and Reservation PASS 0/40, **TwoTier FAIL 14/40** over 67 ms. **Scope: UL only on this workload** — M05/M06 read `frame_completions`, populated only for flows with a `frame_id`, which `sim/traffic.py` assigns only in `_gen_xr_video`; the parametric mix's one DL flow (qfi 82) has no PDU-set structure, so neither arm's DL ranking key can decide anything here. A reader will assume a completeness guarantee covers both directions; it does not. **MECHANISM.** At slot 1 no UE has been granted, so every UL ranking key is identical (`has_gbr` false, `pdb_ms` at the 9999 sentinel) and the stable sort serves declaration order; the PRB budget reaches the first five and those five acquire real QoS state, after which the ones it missed can never compete. The victim receives **0 UL grants in 40,000 slots** while its SR fires **34,560 times**. `n_never_granted > 0` ⟺ M08 floored, **0 counterexamples in 36 runs** — so G5, G10's admissible-fleet boundary and the UL blackout are **one mechanism**. **IN THE DEPLOYED C.** `update_ul_qos_priority` (`gNB_scheduler_ulsch.c:41-70`) seeds 9999/false and `continue`s past every zero per-LCG entry; the array's only two writers are the BSR MAC-CE handlers, which need a grant. The C's Tier-1.5 floor is the designed rescue and its arming gate reads `has_pending_gbr` (`ia_p5g_scheduler.c:2325`) — set only inside that same zero-gated loop. **The remedy cannot arm in the fault it was built for**; measured at 0 firings in 32,000 evaluations per starved UE. **VERDICT: stays a FAILURE.** I withdraw my earlier *"its victim is arbitrary"* — `add_UE_to_list` appends and `remove_UE_from_list` compacts, so `connected_ue_list` is in **attach order** and the victim is the most recent joiner, a real deployment role. Deterministic, total (0 of 299 frames), and product-side. **Falsifier:** if an attach procedure (RACH/Msg3/RRC-setup grants) removed the starvation at every fleet size, G5 should be re-scored — this sim has none, and that is its own commit. |
