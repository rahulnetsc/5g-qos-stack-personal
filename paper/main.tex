...
%%
%% BEFORE SUBMITTING, see paper/README.md -- author block, page limit,
%% anonymisation and artifact-availability policy all still need checking
%% against the current call for papers.
%% ---------------------------------------------------------------------------
...
% TODO: replace with the real author block, and check whether the track is
% double-blind before submitting.
\author{
...
target to track. The natural composition---slow declarative program feeding
a fast tracker---is the pattern behind NVS~\cite{kokku2012} and the O-RAN
non-RT/near-RT RIC split~\cite{oran}. Our contribution is not the pattern
but its concrete instantiation against a symbol-accurate 5G NR resource
model, and the empirical characterisation of when it pays.
...
QoS-awareness rather than grant granularity.
...
The distinction matters for attribution. Cell-edge starvation under
utility-based scheduling is often ascribed to ``weighted-$\log$
pathology''; our measurements locate it instead in the \emph{shortfall
term}, which is what converts a merely SE-favouring allocation into a
vertex one that abandons outright.
...
The simulator, the scheduler library and all scenario configurations are
available at \url{https://github.com/artpark-hub/5g-qos-stack}. Every table
in Section~\ref{sec:results} is regenerated by a single command:
Tables~\ref{tab:sweep}, \ref{tab:sensor} and~\ref{tab:latency} by
\texttt{scheduler\_study.py}; Table~\ref{tab:perflow} by
\texttt{maxmin\_study.py} and \texttt{compare\_schedulers.py};
Table~\ref{tab:knapsack} and the penalty sweep of
Section~\ref{sec:knapsack} by \texttt{maxmin\_study.py}; the sensitivity
results of Section~\ref{sec:sensitivity} by \texttt{bsr\_study.py} and
\texttt{cqi\_study.py}. A dated engineering log recording every finding,
regression and rejected design in this paper is included in the repository.
% TODO: confirm the repository is public before submission.
