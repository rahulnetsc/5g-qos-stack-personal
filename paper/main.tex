...
natural modelling choice, but it is not the common one. We checked the
canonical GBR-scheduling papers directly: none uses a shortfall penalty.
The LTE and NR literature encodes the contract either in a per-flow
\emph{metric} under a priority partition---a PF ratio weighted by a
target-rate factor, with below-target users promoted
wholesale~\cite{monghal2008qos,zaki2011multi,ameigeiras2016qos}---or in
the shape of the \emph{utility function} itself, and such utilities are
deliberately kept \emph{concave} precisely so that the allocation problem
has a unique solution~\cite{gora2014qos}. A strictly concave contract utility has an
