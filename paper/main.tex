...
utility by a factor we measure at $3.4\times10^{7}$. The program is then
...
abandoned, at exactly $0$. We show no reweighting of the penalty can remove
this, and that a max-min satisfaction stage does, lifting the worst-served
flow from 0\% to 40\% of contract at a cost of 4\% of cell throughput.
...
feasible. On our factory workload we measure the two terms at $709$ and
$2.4\times10^{10}$ respectively: a ratio of $3.4\times10^{7}$. That is not
...
29.7 & ue2, ue4, ue9 (16--18\,dB) & 53--56\% \\
21.6 & ue7 (14\,dB) & \textbf{0\%} \\
\bottomrule
\end{tabular}
\end{table}

Two controls confirm the mechanism. Sweeping $p$ over six decades
($1$ to $10^6$) leaves the targets \emph{identical}: the solution is fixed
by the knapsack structure, not the penalty magnitude. Conversely at
$p=10^{-6}$, with the utility alone driving, the allocation is 44--56\% and
\emph{rises} as SE falls---the worst-channel flow receives the largest
fraction of its contract. \textbf{The $\log$ utility is the term that
protects cell-edge flows; the slack penalty is the term that starves
them.} This inverts the usual attribution of cell-edge starvation to
``weighted-$\log$ pathology.''
