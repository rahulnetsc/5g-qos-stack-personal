...
\title{Evaluation of a Two-Tier QoS-Aware Scheduler\\
for Single-Cell Private 5G Networks}
...
\begin{abstract}
Private 5G networks for factories, typically will be covered by a single base
station and will carry traffic whose classes want
...
class-aware gradient baseline. Our main result is that the value of
...
satisfaction, and it decides outcomes only in a moderate-load band,
where it honours 10/10 GBR contracts against PF's 5/10. Two features
account for most of the benefit: configured grants, which bypass both the
control-channel budget and the buffer-status-report round trip and meet
30/30 deadlines where PF meets 2/30; and a deadline-aware per-slot tracker,
without which PF's deadline misses are silent rather than visible. We also
report the design lesson that cost us the most---enforcing rate contracts
through a linear shortfall penalty makes the strategic program abandon the
worst-channel flow outright, which no reweighting of that penalty prevents
and a hard max-min floor does---together with two negative results, and we
release the simulator and all scripts.
\end{abstract}
...
\item \textbf{A structural result on penalty-based contract enforcement}
(Section~\ref{sec:knapsack}). If GBR floors are enforced by a linear
penalty on shortfall---a natural choice, and the one we made---that penalty
outweighs the utility by a factor we measure at $3.3\times10^{7}$. The program is then
...
utility term alone. No reweighting of the penalty removes this; a max-min
satisfaction stage does, lifting the worst-served flow from 0\% to 40\% of
contract at a cost of 4\% of cell throughput. We are explicit that this is
a property of the penalty formulation rather than of QoS-aware scheduling
generally: the more common device in the literature is a \emph{concave}
utility encoding the contract, which is deliberately kept concave for
solution uniqueness~\cite{gora2014qos} and so cannot exhibit it.
...
The distinction matters, and it is worth being precise about its scope.
That utility maximisation favours high-SNR users is
established~\cite{capozzi2013}; our control confirms it rather than
discovering it. What the \emph{shortfall term} adds is the vertex: the
worst flow receives exactly zero rather than merely too little, for every
...
\textbf{This is a property of our formulation, not of QoS-aware scheduling.}
Enforcing a rate contract through a linear penalty on shortfall is a
natural modelling choice, but it is not the common one. The LTE and NR
scheduling literature more often encodes the contract either in a per-flow
\emph{metric}---a PF ratio multiplied by a GBR-deficit
factor~\cite{mongha2008qos,ameigeiras2016qos}---or in the shape of the
\emph{utility function} itself, and such utilities are deliberately kept
\emph{concave} precisely so that the allocation problem has a unique
solution~\cite{gora2014qos}. A strictly concave contract utility has an
interior optimum and therefore cannot abandon a flow the way
Table~\ref{tab:knapsack} shows. We keep the penalty form because the
lexicographic pair of Section~\ref{sec:lex} expresses ``contracts first,
then utility'' exactly where a concave utility only approximates it, and
because the max-min stage then delivers a \emph{guarantee} rather than a
soft preference. But a reader adopting a penalty formulation should know
what it carries, which is the point of this subsection.
