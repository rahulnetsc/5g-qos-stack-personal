"""Product + CG tables: each arm without CG (this morning's aligned
artefacts), with restricted CG (+CG) and with unrestricted CG (+CGu).
Run: uv run python sweeps/cg-2026-09-14/report_tables_cg.py
"""
...
BASE = HERE.parent / "g3-proto" / "rrage-2026-09-14" / "aligned"
...
# ---------------------------------------------------------------- G3
g3 = {"": load(BASE / "g3.json"), "+CG": load(HERE / "g3_cg.json"), "+CGu": load(HERE / "g3_cgu.json")}
if all(g3.values()):
    print("## G3 — product + CG\n")
...
# ---------------------------------------------------------------- G5
g5b, g5r, g5u = load(BASE / "g5.json"), load(HERE / "g5_cg.json"), load(HERE / "g5_cgu.json")
